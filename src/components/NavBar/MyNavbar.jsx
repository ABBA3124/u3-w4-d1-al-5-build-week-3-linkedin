    ...
    <Container id="navbar" style={{ zIndex: "3" }}>
      ...
      {isVisible ? (
        <div id="scroll-element" className={isVisible ? "visible" : "invisible"}>
          <Col className="utente">
            <div>
              ...
              <div className="d-inline-block">
                <p className="fw-bold">{profileData ? `${profileData.name} ${profileData.surname}` : "Nome Utente"}</p>
                <p>{profileData ? profileData.title : "Titolo Utente"}</p>
              </div>
            </div>
          </Col>
          ...
          <Button variant="outline-primary rounded-5  mx-2">Aggiungi sezione profilo</Button>
          <Button variant="primary rounded-5 mx-2 utenteR">Disponibile per</Button>
        </div>
      ) : (
        " "
      )}
